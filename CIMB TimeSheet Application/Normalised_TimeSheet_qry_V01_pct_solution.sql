...
WHERE     (t_act.TimeByDay BETWEEN '01-Feb-2011' AND '01-Mar-2011')
...
WHERE     (TimeByDay BETWEEN '01-Feb-2011' AND '01-Mar-2011') AND
(NOT EXISTS
(SELECT     *
...
SUM(
	(CONVERT(NUMERIC(18,14),#t2.normalizedtime) /(CASE WHEN nortotal.total_Proj = 0 THEN 1.0 ELSE CONVERT(NUMERIC(18,14),nortotal.total_Proj) END)
													)*CONVERT(NUMERIC(18,14),#t4.total_paid_leave) + CONVERT(NUMERIC(18,14),#t2.normalizedtime)
...
AS pct_total_time, #t6.t_type,res_total.res_total_time AS t_proj_time  INTO #t7
...
SELECT * FROM #t6
drop table #t6
select #t7.ResUID, #t7.ProjUID, #t7.ProjName, #t7.total_time, #t7.pct_total_time,#t7.t_proj_time,uv_res.ResourceName,
